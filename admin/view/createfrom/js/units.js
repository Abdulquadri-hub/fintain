...
async function getsmsunits() {
    const request = await httpRequest2('https://comeandsee.com.ng/kreativerock/admin/controllers/units/user', null, null, 'json');
    if(request.ok){
        did('smsunits').innerHTML = FormatNumber(request.data.unit_balance)
        did('smsunitsspent').innerHTML = formatNumber(request.data.unit_spent)
    }else{
...
async function fetchSmsPageData() {
    let request = await httpRequest2('https://comeandsee.com.ng/kreativerock/admin/controllers/packages/sms/fetch', null, null, 'json')
