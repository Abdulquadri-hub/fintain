...
<section class="animate__animated animate__fadeIn">
    ...
    <ul class="flex flex-wrap text-sm font-medium text-center text-gray-500 border-b border-gray-200 dark:border-gray-700 dark:text-gray-400 mb-5">
        <li class="me-2 cursor-pointer updater optioner !text-blue-600 active" name="smspackageform" onclick="runoptioner(this)">
            <p class="inline-block p-4 rounded-t-lg hover:text-gray-600 hover:bg-gray-50">Create Package</p>
        </li>
        <li class="me-2 cursor-pointer viewer optioner" name="viewsmspackages" onclick="runoptioner(this)">
            <p class="inline-block p-4 rounded-t-lg hover:text-gray-600 hover:bg-gray-50">View Packages</p>
        </li>
    </ul>

    <!-- Create Package Form -->
    <form id="smspackageform" class="space-y-6 bg-white/90 p-8 rounded-2xl shadow-xl max-w-full mx-auto">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <!-- Package Name -->
        <div class="form-group">
            <label class="block text-sm font-medium text-gray-700">Package Name</label>
            <input type="text" class="mt-2 p-4 w-full rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:outline-none shadow-sm placeholder-gray-500" name="packagename" id="packagename" placeholder="Enter package name">
        </div>

        <!-- Number of Units -->
        <div class="form-group">
            <label class="block text-sm font-medium text-gray-700">Number of Units</label>
            <input type="number" name="numberofunits" id="numberofunits" class="mt-2 p-4 w-full rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:outline-none shadow-sm placeholder-gray-500">
        </div>

        <!-- Cost per Unit -->
        <div class="form-group">
            <label class="block text-sm font-medium text-gray-700">Cost per Unit</label>
            <input type="number" inputmode="numeric" name="costperunit" id="costperunit" class="mt-2 p-4 w-full rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:outline-none shadow-sm placeholder-gray-500">
        </div>
    </div>

    <!-- Submit Button -->
    <div class="flex justify-end">
        <button type="button" id="submit" class="btn bg-blue-600 hover:bg-blue-700 text-white py-3 px-8 rounded-lg mt-5 transition duration-300 ease-in-out transform hover:scale-105">
  ...
  <!-- Dialog Element -->
  <dialog id="dialogBox" class="rounded-xl shadow-lg w-full max-w-sm p-6">
    <form method="dialog" id="dialogForm" class="space-y-4">
      <h2 class="text-lg font-semibold mb-2">Edit SMS Package Name</h2>
      <input
        type="text"
