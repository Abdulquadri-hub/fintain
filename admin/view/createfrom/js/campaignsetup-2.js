...
async function campaignSetup2Active() {
...
async function createCampaign(btn) {
    ...
    const request = await httpRequest(baseurl+'/campaign/create', payload, btn)
    
    if(!request.status) {
        notification(request.message ?? 'Sorry! Unable to create campaign', 0)
        return
    }else{
        router.navigate('/campaign/managecampaign')
...
function getCampaignFormData() {
    ...
    const segment_id = document.getElementById('segment').tomselect.getValue();
    
    const params = {
        campaigntype: campaignParams.campaignType,
        campaignname: campaignParams.campaignname,
        // sender: senderInput.value.trim(),
        responsehandling: campaignParams.promotionType,
        ...
        channel: campaignParams.campaignMedium
    }
