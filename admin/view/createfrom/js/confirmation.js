...
function appendConfirmationDetails(paymentResponse, paymentInfo) {
    ...
    if(paymentInfo.module === 'sms') {
        details.message = 'Your sms unit payment is complete'
        details.service = paymentInfo.service ?? 'SMS'
        details.paymentHistoryURL = location.origin + BASE_HREF + `/unit/history`
    }else if( paymentInfo.module === 'whatsapp') {
        details.message = 'Your whatsapp unit payment is complete'
        details.service = paymentInfo.service ?? 'WHATSAPP'
        details.paymentHistoryURL = location.origin + BASE_HREF + `/whatsappunit/history`
    }
    ...
    if(historyLink){
      historyLink.addEventListener("click",function(){
        ...
        }else if( paymentInfo.module == 'sms'){
             window.location.href="/whatsappunit/history"
        }
      }) 
    }
}